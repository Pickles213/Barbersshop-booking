...
// Accepts 09XXXXXXXXX or +639XXXXXXXXX / 639XXXXXXXXX and normalizes to +639XXXXXXXXX
...
// Keep only characters relevant to a PH mobile number, capped at a sane length
function sanitizePhoneInput(value: string): string {
  const cleaned = value.replace(/[^\d+]/g, "");
  const maxLen = cleaned.startsWith("+") ? 13 : 11; // +639XXXXXXXXX or 09XXXXXXXXX
...
function AuthPage() {
  ...
  const handleGoogle = async () => {
    ...
    const redirect_uri = `${window.location.origin}${search.redirect ?? "/queue"}`;
    ...
    // in-iframe tokens: navigate
    const { data: u } = await supabase.auth.getUser();
    if (!u.user) { setGoogleLoading(false); return; }
    if (search.redirect) return navigate({ to: search.redirect });
    const { data: role } = await supabase
      .from("user_roles").select("role").eq("user_id", u.user.id).eq("role", "admin").maybeSingle();
    navigate({ to: role ? "/admin/dashboard" : "/queue" });
  };
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (error) return toast.error(error.message);
    // If email confirmation is required, session will be null — show the code entry screen.
